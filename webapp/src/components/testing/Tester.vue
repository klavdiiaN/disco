...
import type { Image, Task, TypedDataset, Features } from "@epfml/discojs";
...
async function modelInference(): Promise<void> {
  ...
  try {
    const generator = await v.inference(toRaw(dropName(props.dataset)));
...
async function testModel(): Promise<void> {
  ...
  try {
    const generator = v.test(dropName(toRaw(props.dataset)));
      ...
      switch (props.dataset[0]) {
        case "image":
          dataWithPred.value = List(
            await arrayFromAsync(toRaw(props.dataset[1])),
          )
...
function saveCsv() {
  ...
  const csvData = format(props.dataset[0], dataWithPred.value);
